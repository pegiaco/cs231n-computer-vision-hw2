...
def batchnorm_backward(dout, cache):
    ...
    dsample_mean = np.sum(dx_hat * (-1/np.sqrt(sample_var + eps)), axis=0) + dsample_var * np.sum(-2 * (x - sample_mean), axis=0) / N
...
def layernorm_backward(dout, cache):
    ...
    dgamma = np.sum(dout * x_hat, axis=0)
    dx_hat = dout * gamma
    dx_hat_sum = np.sum(dx_hat, axis=1, keepdims=True)
    dx = dx_hat - dx_hat_sum / N - np.sum(dx_hat * x_hat, axis=1, keepdims=True) * x_hat / N
        ...
        for c in range(C):
            for i in range(H_out):
                for j in range(W_out):
                    ...
                    max_idx = np.argmax(x[n, c, i*stride:i*stride+pool_height, j*stride:j*stride+pool_width])
                    # unravel the index
                    max_idx = np.unravel_index(max_idx, (pool_height, pool_width))
                    # update dx
                    dx[n, c, i*stride:i*stride+pool_height, j*stride:j*stride+pool_width][max_idx] += dout[n, c, i, j]
...
def spatial_batchnorm_forward(x, gamma, beta, bn_param):
    ...
    # reshape out to (N, H, W, C)
    out = out.reshape(N, H, W, C)

    # transpose out to (N, C, H, W)
...
def spatial_batchnorm_backward(dout, cache):
    ...
    # reshape dx to (N, H, W, C)
    dx = dx.reshape(N, H, W, C)

    # transpose dx to (N, C, H, W)
...
def spatial_groupnorm_forward(x, gamma, beta, G, gn_param):
    ...
    # reshape x to use vanilla layernorm
    x = x.reshape(N*G, -1)
...
def spatial_groupnorm_backward(dout, cache):
    ...
    # dgamma
    dgamma = np.sum(dout * cache[1].reshape(N,C,H,W), axis=(0,2,3), keepdims=True)

    # dbeta
    dbeta = np.sum(dout, axis=(0,2,3), keepdims=True)

    # reshape dout to use vanilla layernorm
    dout = dout.reshape(N*G, -1)

    # perform vanilla layernorm
    dx, _, _ = layernorm_backward(dout, cache)
